<?php
	...
	function ewf_pgContact_metaUpdate() {
		...
		if (is_object($post) && array_key_exists('_ewf-contact-address', $_POST)){
				
			$contact_meta = get_post_custom($post->ID);
			$contact_address = sanitize_text_field( $_POST["_ewf-contact-address"] );
			$contact_zoom = intval( $_POST["_ewf-contact-zoom"] );
			$contact_caption = sanitize_text_field( $_POST["_ewf-contact-caption"] );
