...
{ 
	...
	function meta_update(){
			...
			if (array_key_exists('_ewf_modHeader_border_color', $_POST)){
				$post_modHeader_settings['border_color'] = sanitize_text_field( $_POST['_ewf_modHeader_border_color'] );
			}	
			
			if (array_key_exists('_ewf_modHeader_font_color', $_POST)){
				$post_modHeader_settings['font_color'] = sanitize_text_field( $_POST['_ewf_modHeader_font_color'] );
			}
		...
		function meta_source(){
			...
			$ewf_modHeader_settings = $this->get_postSettings($post->ID, true);
			$cache_pages = $this->cache_getPages();
				
			if ($ewf_modHeader_settings['active'] === '*'){
				$ewf_modHeader_active = 1;
				$ewf_modHeader_settings['debug'][] = '# Header auto active';
			}else{
			...
			$ewf_modHeader_settings['debug'][] = '# Use page header toggle ['.$ewf_modHeader_settings['active'].']['.$ewf_modHeader_active.'] '.$ewf_modHeader_activeContent_class.' '.$ewf_modHeader_active_class;
			...
			echo '<div class="ewf-modHeader-metaBox">';		
					...
					echo '<div class="ewf-mh-tabscontent clearfix '.$ewf_modHeader_activeContent_class.'">';
						...
						$ewf_modHeader_borderColorClass = null;
						$ewf_modHeader_fontColorClass = null;
						...
						echo '<div class="ewf-mh-tbcontent-header tab-active">';
							...
							if (array_key_exists('master_use', $ewf_modHeader_settings) && $ewf_modHeader_settings['master_use'] == 1){
								...
								$ewf_modHeader_borderColorClass = ' disabled';
								$ewf_modHeader_fontColorClass = ' disabled';
							...
							echo '<div class="ewf-mh-stgroup">';
								...
								if (current_theme_supports('ewf-modHeader-background-color')){	
									...
									if ($ewf_modHeader_backgroundColorClass && $ewf_master_settings['background_color']){
										...
										$ewf_modHeader_settings['debug'][] = '# Master Background Color:'.$ewf_master_settings['background_color'];	// DEBUG
									}
								...
								//	Page header - Border color field
								//	
								if (current_theme_supports('ewf-modHeader-border-color')){	
								
									if (!array_key_exists('border_color', $ewf_modHeader_settings)){	
										$ewf_modHeader_settings['border_color'] = $this->settings['defaults']['border_color'];
									}
								
									$ewf_modHeader_borderColor = $ewf_modHeader_settings['border_color'];
									
									if ( $ewf_modHeader_borderColorClass && $ewf_master_settings['border_color']){
										$ewf_modHeader_settings['debug'][] = '# Master Border Color:'.$ewf_master_settings['border_color'];	// DEBUG
										$ewf_modHeader_borderColor = $ewf_master_settings['border_color'];
										
									}

									// echo '<br/>Master: '.$ewf_master_settings['border_color'];
									/*
									echo '<pre>Settings:';
										print_r($ewf_modHeader_settings);
										echo'<br><br/>Master:';
										print_r($ewf_master_settings);
									echo '</pre>';
									*/
									
									echo '<div class="ewf-mh-stfield mh-field-color clearfix'.$ewf_modHeader_borderColorClass.'" data-color="'.$ewf_modHeader_settings['border_color'].'">';					
										echo '<label>'.__('Border color', EWF_SETUP_THEME_DOMAIN).'</label>';
										echo '<input name="_ewf_modHeader_border_color" type="text" id="_ewf_modHeader_border_color" value="'.$ewf_modHeader_borderColor.'" data-default-color="#000" >';
										echo '<div class="disabled-overlay"></div>'; 
									echo '</div>';
								}
								...
								if (current_theme_supports('ewf-modHeader-font-color')){	
									...
									if (!array_key_exists('font_color', $ewf_modHeader_settings)){	
										$ewf_modHeader_settings['font_color'] = $this->settings['defaults']['font_color'];
									}
								
									$ewf_modHeader_fontColor = $ewf_modHeader_settings['font_color'];
									
									if ( $ewf_modHeader_fontColorClass && $ewf_master_settings['font_color']){
										$ewf_modHeader_settings['debug'][] = '# Master Font Color:'.$ewf_master_settings['font_color'];	// DEBUG
										$ewf_modHeader_fontColor = $ewf_master_settings['font_color'];
										
									}

									// echo '<br/>Master: '.$ewf_master_settings['font_color'];
									/*
									*/
									// echo '<pre>Settings:';
										// print_r($ewf_modHeader_settings);
									...
									echo '<div class="ewf-mh-stfield mh-field-color clearfix'.$ewf_modHeader_fontColorClass.'" data-color="'.$ewf_modHeader_settings['font_color'].'">';
										echo '<label>'.__('Font color', EWF_SETUP_THEME_DOMAIN).'</label>';
										echo '<input name="_ewf_modHeader_font_color" type="text" id="_ewf_modHeader_font_color" value="'.$ewf_modHeader_fontColor.'" data-default-color="#000" >';
										echo '<div class="disabled-overlay"></div>'; 
									echo '</div>';
								}
								...
								if (current_theme_supports('ewf-modHeader-parallax')){
									...
									echo '</div>';
									 ...
									 if (current_theme_supports('ewf-modHeader-master')){
				   ...
				   echo '</div>';
					...
					ewf_debug($ewf_modHeader_settings);
		...
		function generate_inputTextarea($prop, $post_custom_meta, $cache_pages){
			...
			$src = '<div class="ewf-mh-stfield '.$field_type_value.' '.$field_master_value.' no-dropdown clearfix">
						<div class="ewf-mh-fielddrop">
					...
					$src.= '<select autocomplete="off" name="'.$prop->field_id.'" id="'.$prop->field_id.'">';
						#	$src .='<option value="0">None</option>';
							...
							foreach($cache_pages as $key => $page_item){
								...
								$export .= '<a data-excerpt="'.esc_attr($page_item['excerpt']).'" data-id="'.$page_item['id'].'" data-image="'.$page_item['image_url'].'" data-background-color="'.$page_item['background_color'].'"  data-border-color="'.$page_item['border_color'].'" data-title="'.esc_attr($page_item['title']).'" data-header-title="'.esc_attr($page_item['header_title']).'">Page</a>, ';
							}
				$src .= '</select>
		...
		function set_postSettings($post_modHeader_settings, $post_id){
			...
			return update_post_meta($post_id, '_ewf_modHeader_settings', $post_modHeader_settings);
			
			/*
			if (is_serialized($post_modHeader_settings)){
				return update_post_meta($post_id, '_ewf_modHeader_settings', $post_modHeader_settings);
			}else{
				return update_post_meta($post_id, '_ewf_modHeader_settings', serialize($post_modHeader_settings));
			}
			*/
			
		}
		
		function get_postSettings($post_id = 0, $debug = false){
			global $post;
			...
			if (!$post_id) {
				if (is_object($post)){
					$post_id = $post->ID;
					$ewf_modHeader_debug[] = 'No Post ID';
				}else{
					$post_modHeader_settings = $post_modHeader_defaults;
					$ewf_modHeader_debug[] = 'Post not object: '.$post_id.' - Return defaults';
				}
			}
			
			// $post_meta = get_post_custom($post_id);
			$post_modHeader_settings = get_post_meta( $post_id, '_ewf_modHeader_settings', true );
			$ewf_modHeader_debug[] = 'post ID: '.$post_id;
			
			
			// if (array_key_exists('_ewf_modHeader_settings', $post_meta)){
			if (empty($post_modHeader_settings)){
				$post_modHeader_settings = $post_modHeader_defaults;
				$ewf_modHeader_debug[] = 'Meta not found: '.$post_id.' - Return defaults';
			}else{
				// $post_modHeader_settings = json_decode($post_meta['_ewf_modHeader_settings'][0], true);
				// $post_modHeader_settings =  $post_meta['_ewf_modHeader_settings'][0];
				// $post_modHeader_settings = unserialize($post_meta['_ewf_modHeader_settings'][0]); 
				$ewf_modHeader_debug[] = 'Post: '.$post_id.' - Return Unserialized';
			}
			
			/*
			if ($debug){
				echo '<pre>';
					print_r($ewf_modHeader_debug);
		...
		function cache_getPages($exclude_id = 0, $filter = false){
			...
			foreach($pages_list as $current_page){
				...
				if (is_array($post_modHeader_settings)){
					...
					if (array_key_exists('border_color', $post_modHeader_settings)){
						$current_page_data['border_color'] = $post_modHeader_settings['border_color'];
					}
				
					if (array_key_exists('font_color', $post_modHeader_settings)){
						$current_page_data['font_color'] = $post_modHeader_settings['font_color'];
					}
