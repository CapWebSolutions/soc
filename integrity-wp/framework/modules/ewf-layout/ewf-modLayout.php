 ...
 */
	...
	$ewf_modLayout_settings = array(
		...
		'sidebars' => array(
			...
			'install' => array(
				...
				'sidebar-shop'		=> array( 'title' => __('WooCommerce Sidebar'	, EWF_SETUP_THEME_DOMAIN), 'description' => null ),
	...
	function ewf_get_page_relatedID( $headerCheck = false, $page_for_posts = 0 ){
		...
		if (function_exists('is_shop') && is_product()){
			...
			$ewf_page_id = woocommerce_get_page_id('shop');
		...
		}elseif(function_exists('is_shop') && (is_shop() || is_product_category() || is_product_tag() || is_woocommerce() || is_checkout() )){
			
			// ewf_debug("Related ID: Shop page");
			$ewf_page_id = woocommerce_get_page_id('shop');
